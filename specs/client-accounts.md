...
- [ ] `app/api/documents/[id]/route.ts` (GET/DELETE a single document by ID) — **missed by `feat/phase-1-account-scoping`**, not actually covered by the item above despite matching its glob. Zero account scoping: any session can read or delete any account's document by ID. Tracked as `TECH_DEBT.md` §1.2 — a real gap, not deferred scope like the ingest/analysis items below.
